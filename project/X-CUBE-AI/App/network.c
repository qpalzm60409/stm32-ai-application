/**
  ******************************************************************************
  * @file    network.c
  * @author  AST Embedded Analytics Research Platform
  * @date    Thu Apr  2 10:14:23 2020
  * @brief   AI Tool Automatic Code Generator for Embedded NN computing
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2018 STMicroelectronics.
  * All rights reserved.
  *
  * This software component is licensed by ST under Ultimate Liberty license
  * SLA0044, the "License"; You may not use this file except in compliance with
  * the License. You may obtain a copy of the License at:
  *                             www.st.com/SLA0044
  *
  ******************************************************************************
  */


#include "network.h"

#include "ai_platform_interface.h"
#include "ai_math_helpers.h"

#include "core_common.h"
#include "layers.h"

#undef AI_TOOLS_VERSION_MAJOR
#undef AI_TOOLS_VERSION_MINOR
#undef AI_TOOLS_VERSION_MICRO
#define AI_TOOLS_VERSION_MAJOR 5
#define AI_TOOLS_VERSION_MINOR 0
#define AI_TOOLS_VERSION_MICRO 0


#undef AI_TOOLS_API_VERSION_MAJOR
#undef AI_TOOLS_API_VERSION_MINOR
#undef AI_TOOLS_API_VERSION_MICRO
#define AI_TOOLS_API_VERSION_MAJOR 1
#define AI_TOOLS_API_VERSION_MINOR 3
#define AI_TOOLS_API_VERSION_MICRO 0

#undef AI_NET_OBJ_INSTANCE
#define AI_NET_OBJ_INSTANCE g_network
 
#undef AI_NETWORK_MODEL_SIGNATURE
#define AI_NETWORK_MODEL_SIGNATURE     "692ee428cea146282d633f7266cef950"

#ifndef AI_TOOLS_REVISION_ID
#define AI_TOOLS_REVISION_ID     "(rev-5.0.0)"
#endif

#undef AI_TOOLS_DATE_TIME
#define AI_TOOLS_DATE_TIME   "Thu Apr  2 10:14:23 2020"

#undef AI_TOOLS_COMPILE_TIME
#define AI_TOOLS_COMPILE_TIME    __DATE__ " " __TIME__

#undef AI_NETWORK_N_BATCHES
#define AI_NETWORK_N_BATCHES         (1)

/**  Forward network declaration section  *************************************/
AI_STATIC ai_network AI_NET_OBJ_INSTANCE;


/**  Forward network array declarations  **************************************/
AI_STATIC ai_array conv3_scratch0_array;   /* Array #0 */
AI_STATIC ai_array conv1_scratch0_array;   /* Array #1 */
AI_STATIC ai_array fco_bias_array;   /* Array #2 */
AI_STATIC ai_array fco_weights_array;   /* Array #3 */
AI_STATIC ai_array fc1_bias_array;   /* Array #4 */
AI_STATIC ai_array fc1_weights_array;   /* Array #5 */
AI_STATIC ai_array conv3_bias_array;   /* Array #6 */
AI_STATIC ai_array conv3_weights_array;   /* Array #7 */
AI_STATIC ai_array conv2_bias_array;   /* Array #8 */
AI_STATIC ai_array conv2_weights_array;   /* Array #9 */
AI_STATIC ai_array conv1_bias_array;   /* Array #10 */
AI_STATIC ai_array conv1_weights_array;   /* Array #11 */
AI_STATIC ai_array conv0_bias_array;   /* Array #12 */
AI_STATIC ai_array conv0_weights_array;   /* Array #13 */
AI_STATIC ai_array input_1_output_array;   /* Array #14 */
AI_STATIC ai_array conv0_output_array;   /* Array #15 */
AI_STATIC ai_array conv1_output_array;   /* Array #16 */
AI_STATIC ai_array conv2_output_array;   /* Array #17 */
AI_STATIC ai_array conv3_output_array;   /* Array #18 */
AI_STATIC ai_array fc1_output_array;   /* Array #19 */
AI_STATIC ai_array fc1_nl_output_array;   /* Array #20 */
AI_STATIC ai_array fco_output_array;   /* Array #21 */
AI_STATIC ai_array fco_nl_output_array;   /* Array #22 */


/**  Forward network tensor declarations  *************************************/
AI_STATIC ai_tensor conv3_scratch0;   /* Tensor #0 */
AI_STATIC ai_tensor conv1_scratch0;   /* Tensor #1 */
AI_STATIC ai_tensor fco_bias;   /* Tensor #2 */
AI_STATIC ai_tensor fco_weights;   /* Tensor #3 */
AI_STATIC ai_tensor fc1_bias;   /* Tensor #4 */
AI_STATIC ai_tensor fc1_weights;   /* Tensor #5 */
AI_STATIC ai_tensor conv3_bias;   /* Tensor #6 */
AI_STATIC ai_tensor conv3_weights;   /* Tensor #7 */
AI_STATIC ai_tensor conv2_bias;   /* Tensor #8 */
AI_STATIC ai_tensor conv2_weights;   /* Tensor #9 */
AI_STATIC ai_tensor conv1_bias;   /* Tensor #10 */
AI_STATIC ai_tensor conv1_weights;   /* Tensor #11 */
AI_STATIC ai_tensor conv0_bias;   /* Tensor #12 */
AI_STATIC ai_tensor conv0_weights;   /* Tensor #13 */
AI_STATIC ai_tensor input_1_output;   /* Tensor #14 */
AI_STATIC ai_tensor conv0_output;   /* Tensor #15 */
AI_STATIC ai_tensor conv1_output;   /* Tensor #16 */
AI_STATIC ai_tensor conv2_output;   /* Tensor #17 */
AI_STATIC ai_tensor conv3_output;   /* Tensor #18 */
AI_STATIC ai_tensor conv3_output0;   /* Tensor #19 */
AI_STATIC ai_tensor fc1_output;   /* Tensor #20 */
AI_STATIC ai_tensor fc1_nl_output;   /* Tensor #21 */
AI_STATIC ai_tensor fco_output;   /* Tensor #22 */
AI_STATIC ai_tensor fco_nl_output;   /* Tensor #23 */


/**  Forward network tensor chain declarations  *******************************/
AI_STATIC_CONST ai_tensor_chain conv0_chain;   /* Chain #0 */
AI_STATIC_CONST ai_tensor_chain conv1_chain;   /* Chain #1 */
AI_STATIC_CONST ai_tensor_chain conv2_chain;   /* Chain #2 */
AI_STATIC_CONST ai_tensor_chain conv3_chain;   /* Chain #3 */
AI_STATIC_CONST ai_tensor_chain fc1_chain;   /* Chain #4 */
AI_STATIC_CONST ai_tensor_chain fc1_nl_chain;   /* Chain #5 */
AI_STATIC_CONST ai_tensor_chain fco_chain;   /* Chain #6 */
AI_STATIC_CONST ai_tensor_chain fco_nl_chain;   /* Chain #7 */


/**  Forward network layer declarations  **************************************/
AI_STATIC ai_layer_conv2d conv0_layer; /* Layer #0 */
AI_STATIC ai_layer_conv2d_nl_pool conv1_layer; /* Layer #1 */
AI_STATIC ai_layer_conv2d conv2_layer; /* Layer #2 */
AI_STATIC ai_layer_conv2d_nl_pool conv3_layer; /* Layer #3 */
AI_STATIC ai_layer_dense fc1_layer; /* Layer #4 */
AI_STATIC ai_layer_nl fc1_nl_layer; /* Layer #5 */
AI_STATIC ai_layer_dense fco_layer; /* Layer #6 */
AI_STATIC ai_layer_nl fco_nl_layer; /* Layer #7 */


/**  Array declarations section  **********************************************/
AI_ARRAY_OBJ_DECLARE(
    conv3_scratch0_array, AI_ARRAY_FORMAT_FLOAT,
    NULL, NULL, 1024,
     AI_STATIC)
AI_ARRAY_OBJ_DECLARE(
    conv1_scratch0_array, AI_ARRAY_FORMAT_FLOAT,
    NULL, NULL, 1536,
     AI_STATIC)
AI_ARRAY_OBJ_DECLARE(
    fco_bias_array, AI_ARRAY_FORMAT_FLOAT,
    NULL, NULL, 10,
     AI_STATIC)
AI_ARRAY_OBJ_DECLARE(
    fco_weights_array, AI_ARRAY_FORMAT_FLOAT,
    NULL, NULL, 2560,
     AI_STATIC)
AI_ARRAY_OBJ_DECLARE(
    fc1_bias_array, AI_ARRAY_FORMAT_FLOAT,
    NULL, NULL, 256,
     AI_STATIC)
AI_ARRAY_OBJ_DECLARE(
    fc1_weights_array, AI_ARRAY_FORMAT_LUT4_FLOAT,
    NULL, NULL, 262144,
     AI_STATIC)
AI_ARRAY_OBJ_DECLARE(
    conv3_bias_array, AI_ARRAY_FORMAT_FLOAT,
    NULL, NULL, 64,
     AI_STATIC)
AI_ARRAY_OBJ_DECLARE(
    conv3_weights_array, AI_ARRAY_FORMAT_FLOAT,
    NULL, NULL, 36864,
     AI_STATIC)
AI_ARRAY_OBJ_DECLARE(
    conv2_bias_array, AI_ARRAY_FORMAT_FLOAT,
    NULL, NULL, 64,
     AI_STATIC)
AI_ARRAY_OBJ_DECLARE(
    conv2_weights_array, AI_ARRAY_FORMAT_FLOAT,
    NULL, NULL, 18432,
     AI_STATIC)
AI_ARRAY_OBJ_DECLARE(
    conv1_bias_array, AI_ARRAY_FORMAT_FLOAT,
    NULL, NULL, 32,
     AI_STATIC)
AI_ARRAY_OBJ_DECLARE(
    conv1_weights_array, AI_ARRAY_FORMAT_FLOAT,
    NULL, NULL, 9216,
     AI_STATIC)
AI_ARRAY_OBJ_DECLARE(
    conv0_bias_array, AI_ARRAY_FORMAT_FLOAT,
    NULL, NULL, 32,
     AI_STATIC)
AI_ARRAY_OBJ_DECLARE(
    conv0_weights_array, AI_ARRAY_FORMAT_FLOAT,
    NULL, NULL, 288,
     AI_STATIC)
AI_ARRAY_OBJ_DECLARE(
    input_1_output_array, AI_ARRAY_FORMAT_FLOAT|AI_FMT_FLAG_IS_IO,
    NULL, NULL, 784,
     AI_STATIC)
AI_ARRAY_OBJ_DECLARE(
    conv0_output_array, AI_ARRAY_FORMAT_FLOAT,
    NULL, NULL, 21632,
     AI_STATIC)
AI_ARRAY_OBJ_DECLARE(
    conv1_output_array, AI_ARRAY_FORMAT_FLOAT,
    NULL, NULL, 4608,
     AI_STATIC)
AI_ARRAY_OBJ_DECLARE(
    conv2_output_array, AI_ARRAY_FORMAT_FLOAT,
    NULL, NULL, 6400,
     AI_STATIC)
AI_ARRAY_OBJ_DECLARE(
    conv3_output_array, AI_ARRAY_FORMAT_FLOAT,
    NULL, NULL, 1024,
     AI_STATIC)
AI_ARRAY_OBJ_DECLARE(
    fc1_output_array, AI_ARRAY_FORMAT_FLOAT,
    NULL, NULL, 256,
     AI_STATIC)
AI_ARRAY_OBJ_DECLARE(
    fc1_nl_output_array, AI_ARRAY_FORMAT_FLOAT,
    NULL, NULL, 256,
     AI_STATIC)
AI_ARRAY_OBJ_DECLARE(
    fco_output_array, AI_ARRAY_FORMAT_FLOAT,
    NULL, NULL, 10,
     AI_STATIC)
AI_ARRAY_OBJ_DECLARE(
    fco_nl_output_array, AI_ARRAY_FORMAT_FLOAT|AI_FMT_FLAG_IS_IO,
    NULL, NULL, 10,
     AI_STATIC)




/**  Tensor declarations section  *********************************************/
AI_TENSOR_OBJ_DECLARE(
  conv3_scratch0, AI_STATIC,
  0x0, 0x0, AI_SHAPE_INIT(4, 1, 64, 8, 2), AI_STRIDE_INIT(4, 4, 4, 256, 2048),
  1, &conv3_scratch0_array, NULL)
AI_TENSOR_OBJ_DECLARE(
  conv1_scratch0, AI_STATIC,
  0x0, 0x0, AI_SHAPE_INIT(4, 1, 32, 24, 2), AI_STRIDE_INIT(4, 4, 4, 128, 3072),
  1, &conv1_scratch0_array, NULL)
AI_TENSOR_OBJ_DECLARE(
  fco_bias, AI_STATIC,
  0x0, 0x0, AI_SHAPE_INIT(4, 1, 10, 1, 1), AI_STRIDE_INIT(4, 4, 4, 40, 40),
  1, &fco_bias_array, NULL)
AI_TENSOR_OBJ_DECLARE(
  fco_weights, AI_STATIC,
  0x0, 0x0, AI_SHAPE_INIT(4, 256, 10, 1, 1), AI_STRIDE_INIT(4, 4, 1024, 10240, 10240),
  1, &fco_weights_array, NULL)
AI_TENSOR_OBJ_DECLARE(
  fc1_bias, AI_STATIC,
  0x0, 0x0, AI_SHAPE_INIT(4, 1, 256, 1, 1), AI_STRIDE_INIT(4, 4, 4, 1024, 1024),
  1, &fc1_bias_array, NULL)
AI_TENSOR_OBJ_DECLARE(
  fc1_weights, AI_STATIC,
  0x0, 0x0, AI_SHAPE_INIT(4, 1024, 256, 1, 1), AI_STRIDE_INIT(4, 1, 512, 131072, 131072),
  1, &fc1_weights_array, NULL)
AI_TENSOR_OBJ_DECLARE(
  conv3_bias, AI_STATIC,
  0x0, 0x0, AI_SHAPE_INIT(4, 1, 64, 1, 1), AI_STRIDE_INIT(4, 4, 4, 256, 256),
  1, &conv3_bias_array, NULL)
AI_TENSOR_OBJ_DECLARE(
  conv3_weights, AI_STATIC,
  0x0, 0x0, AI_SHAPE_INIT(4, 64, 3, 3, 64), AI_STRIDE_INIT(4, 4, 256, 768, 2304),
  1, &conv3_weights_array, NULL)
AI_TENSOR_OBJ_DECLARE(
  conv2_bias, AI_STATIC,
  0x0, 0x0, AI_SHAPE_INIT(4, 1, 64, 1, 1), AI_STRIDE_INIT(4, 4, 4, 256, 256),
  1, &conv2_bias_array, NULL)
AI_TENSOR_OBJ_DECLARE(
  conv2_weights, AI_STATIC,
  0x0, 0x0, AI_SHAPE_INIT(4, 32, 3, 3, 64), AI_STRIDE_INIT(4, 4, 128, 384, 1152),
  1, &conv2_weights_array, NULL)
AI_TENSOR_OBJ_DECLARE(
  conv1_bias, AI_STATIC,
  0x0, 0x0, AI_SHAPE_INIT(4, 1, 32, 1, 1), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &conv1_bias_array, NULL)
AI_TENSOR_OBJ_DECLARE(
  conv1_weights, AI_STATIC,
  0x0, 0x0, AI_SHAPE_INIT(4, 32, 3, 3, 32), AI_STRIDE_INIT(4, 4, 128, 384, 1152),
  1, &conv1_weights_array, NULL)
AI_TENSOR_OBJ_DECLARE(
  conv0_bias, AI_STATIC,
  0x0, 0x0, AI_SHAPE_INIT(4, 1, 32, 1, 1), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &conv0_bias_array, NULL)
AI_TENSOR_OBJ_DECLARE(
  conv0_weights, AI_STATIC,
  0x0, 0x0, AI_SHAPE_INIT(4, 1, 3, 3, 32), AI_STRIDE_INIT(4, 4, 4, 12, 36),
  1, &conv0_weights_array, NULL)
AI_TENSOR_OBJ_DECLARE(
  input_1_output, AI_STATIC,
  0x0, 0x0, AI_SHAPE_INIT(4, 1, 1, 28, 28), AI_STRIDE_INIT(4, 4, 4, 4, 112),
  1, &input_1_output_array, NULL)
AI_TENSOR_OBJ_DECLARE(
  conv0_output, AI_STATIC,
  0x0, 0x0, AI_SHAPE_INIT(4, 1, 32, 26, 26), AI_STRIDE_INIT(4, 4, 4, 128, 3328),
  1, &conv0_output_array, NULL)
AI_TENSOR_OBJ_DECLARE(
  conv1_output, AI_STATIC,
  0x0, 0x0, AI_SHAPE_INIT(4, 1, 32, 12, 12), AI_STRIDE_INIT(4, 4, 4, 128, 1536),
  1, &conv1_output_array, NULL)
AI_TENSOR_OBJ_DECLARE(
  conv2_output, AI_STATIC,
  0x0, 0x0, AI_SHAPE_INIT(4, 1, 64, 10, 10), AI_STRIDE_INIT(4, 4, 4, 256, 2560),
  1, &conv2_output_array, NULL)
AI_TENSOR_OBJ_DECLARE(
  conv3_output, AI_STATIC,
  0x0, 0x0, AI_SHAPE_INIT(4, 1, 64, 4, 4), AI_STRIDE_INIT(4, 4, 4, 256, 1024),
  1, &conv3_output_array, NULL)
AI_TENSOR_OBJ_DECLARE(
  conv3_output0, AI_STATIC,
  0x0, 0x0, AI_SHAPE_INIT(4, 1, 1024, 1, 1), AI_STRIDE_INIT(4, 4, 4, 4096, 4096),
  1, &conv3_output_array, NULL)
AI_TENSOR_OBJ_DECLARE(
  fc1_output, AI_STATIC,
  0x0, 0x0, AI_SHAPE_INIT(4, 1, 256, 1, 1), AI_STRIDE_INIT(4, 4, 4, 1024, 1024),
  1, &fc1_output_array, NULL)
AI_TENSOR_OBJ_DECLARE(
  fc1_nl_output, AI_STATIC,
  0x0, 0x0, AI_SHAPE_INIT(4, 1, 256, 1, 1), AI_STRIDE_INIT(4, 4, 4, 1024, 1024),
  1, &fc1_nl_output_array, NULL)
AI_TENSOR_OBJ_DECLARE(
  fco_output, AI_STATIC,
  0x0, 0x0, AI_SHAPE_INIT(4, 1, 10, 1, 1), AI_STRIDE_INIT(4, 4, 4, 40, 40),
  1, &fco_output_array, NULL)
AI_TENSOR_OBJ_DECLARE(
  fco_nl_output, AI_STATIC,
  0x0, 0x0, AI_SHAPE_INIT(4, 1, 10, 1, 1), AI_STRIDE_INIT(4, 4, 4, 40, 40),
  1, &fco_nl_output_array, NULL)


/**  Layer declarations section  **********************************************/


AI_TENSOR_CHAIN_OBJ_DECLARE(
  conv0_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_ENTRY(&input_1_output),
  AI_TENSOR_LIST_ENTRY(&conv0_output),
  AI_TENSOR_LIST_ENTRY(&conv0_weights, &conv0_bias, NULL),
  AI_TENSOR_LIST_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  conv0_layer, 1,
  CONV2D_TYPE,
  conv2d, forward_conv2d,
  &AI_NET_OBJ_INSTANCE, &conv1_layer, AI_STATIC,
  .tensors = &conv0_chain, 
  .groups = 1, 
  .nl_func = nl_func_relu_array_f32, 
  .filter_stride = AI_SHAPE_2D_INIT(1, 1), 
  .dilation = AI_SHAPE_2D_INIT(1, 1), 
  .filter_pad = AI_SHAPE_INIT(4, 0, 0, 0, 0), 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  conv1_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_ENTRY(&conv0_output),
  AI_TENSOR_LIST_ENTRY(&conv1_output),
  AI_TENSOR_LIST_ENTRY(&conv1_weights, &conv1_bias, NULL),
  AI_TENSOR_LIST_ENTRY(&conv1_scratch0)
)

AI_LAYER_OBJ_DECLARE(
  conv1_layer, 4,
  OPTIMIZED_CONV2D_TYPE,
  conv2d_nl_pool, forward_conv2d_nl_pool,
  &AI_NET_OBJ_INSTANCE, &conv2_layer, AI_STATIC,
  .tensors = &conv1_chain, 
  .groups = 1, 
  .nl_func = nl_func_relu_array_f32, 
  .filter_stride = AI_SHAPE_2D_INIT(1, 1), 
  .dilation = AI_SHAPE_2D_INIT(1, 1), 
  .filter_pad = AI_SHAPE_INIT(4, 0, 0, 0, 0), 
  .pool_size = AI_SHAPE_2D_INIT(2, 2), 
  .pool_stride = AI_SHAPE_2D_INIT(2, 2), 
  .pool_pad = AI_SHAPE_INIT(4, 0, 0, 0, 0), 
  .pool_func = pool_func_mp_array_f32, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  conv2_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_ENTRY(&conv1_output),
  AI_TENSOR_LIST_ENTRY(&conv2_output),
  AI_TENSOR_LIST_ENTRY(&conv2_weights, &conv2_bias, NULL),
  AI_TENSOR_LIST_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  conv2_layer, 8,
  CONV2D_TYPE,
  conv2d, forward_conv2d,
  &AI_NET_OBJ_INSTANCE, &conv3_layer, AI_STATIC,
  .tensors = &conv2_chain, 
  .groups = 1, 
  .nl_func = nl_func_relu_array_f32, 
  .filter_stride = AI_SHAPE_2D_INIT(1, 1), 
  .dilation = AI_SHAPE_2D_INIT(1, 1), 
  .filter_pad = AI_SHAPE_INIT(4, 0, 0, 0, 0), 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  conv3_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_ENTRY(&conv2_output),
  AI_TENSOR_LIST_ENTRY(&conv3_output),
  AI_TENSOR_LIST_ENTRY(&conv3_weights, &conv3_bias, NULL),
  AI_TENSOR_LIST_ENTRY(&conv3_scratch0)
)

AI_LAYER_OBJ_DECLARE(
  conv3_layer, 11,
  OPTIMIZED_CONV2D_TYPE,
  conv2d_nl_pool, forward_conv2d_nl_pool,
  &AI_NET_OBJ_INSTANCE, &fc1_layer, AI_STATIC,
  .tensors = &conv3_chain, 
  .groups = 1, 
  .nl_func = nl_func_relu_array_f32, 
  .filter_stride = AI_SHAPE_2D_INIT(1, 1), 
  .dilation = AI_SHAPE_2D_INIT(1, 1), 
  .filter_pad = AI_SHAPE_INIT(4, 0, 0, 0, 0), 
  .pool_size = AI_SHAPE_2D_INIT(2, 2), 
  .pool_stride = AI_SHAPE_2D_INIT(2, 2), 
  .pool_pad = AI_SHAPE_INIT(4, 0, 0, 0, 0), 
  .pool_func = pool_func_mp_array_f32, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  fc1_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_ENTRY(&conv3_output0),
  AI_TENSOR_LIST_ENTRY(&fc1_output),
  AI_TENSOR_LIST_ENTRY(&fc1_weights, &fc1_bias),
  AI_TENSOR_LIST_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  fc1_layer, 17,
  DENSE_TYPE,
  dense, forward_dense,
  &AI_NET_OBJ_INSTANCE, &fc1_nl_layer, AI_STATIC,
  .tensors = &fc1_chain, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  fc1_nl_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_ENTRY(&fc1_output),
  AI_TENSOR_LIST_ENTRY(&fc1_nl_output),
  AI_TENSOR_LIST_EMPTY,
  AI_TENSOR_LIST_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  fc1_nl_layer, 17,
  NL_TYPE,
  nl, forward_relu,
  &AI_NET_OBJ_INSTANCE, &fco_layer, AI_STATIC,
  .tensors = &fc1_nl_chain, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  fco_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_ENTRY(&fc1_nl_output),
  AI_TENSOR_LIST_ENTRY(&fco_output),
  AI_TENSOR_LIST_ENTRY(&fco_weights, &fco_bias),
  AI_TENSOR_LIST_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  fco_layer, 19,
  DENSE_TYPE,
  dense, forward_dense,
  &AI_NET_OBJ_INSTANCE, &fco_nl_layer, AI_STATIC,
  .tensors = &fco_chain, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  fco_nl_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_ENTRY(&fco_output),
  AI_TENSOR_LIST_ENTRY(&fco_nl_output),
  AI_TENSOR_LIST_EMPTY,
  AI_TENSOR_LIST_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  fco_nl_layer, 19,
  NL_TYPE,
  nl, forward_sm,
  &AI_NET_OBJ_INSTANCE, &fco_nl_layer, AI_STATIC,
  .tensors = &fco_nl_chain, 
)


AI_NETWORK_OBJ_DECLARE(
  AI_NET_OBJ_INSTANCE, AI_STATIC,
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_U8,
                     1, 1, 402408, 1,
                     NULL),
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_U8,
                     1, 1, 94336, 1,
                     NULL),
  AI_TENSOR_LIST_IO_ENTRY(AI_FLAG_NONE, AI_NETWORK_IN_NUM, &input_1_output),
  AI_TENSOR_LIST_IO_ENTRY(AI_FLAG_NONE, AI_NETWORK_OUT_NUM, &fco_nl_output),
  &conv0_layer, 0, NULL)



AI_DECLARE_STATIC
ai_bool network_configure_activations(
  ai_network* net_ctx, const ai_buffer* activation_buffer)
{
  AI_ASSERT(net_ctx &&  activation_buffer && activation_buffer->data)

  ai_ptr activations = AI_PTR(AI_PTR_ALIGN(activation_buffer->data, 4));
  AI_ASSERT(activations)
  AI_UNUSED(net_ctx)

  {
    /* Updating activations (byte) offsets */
    conv3_scratch0_array.data = AI_PTR(activations + 88192);
    conv3_scratch0_array.data_start = AI_PTR(activations + 88192);
    conv1_scratch0_array.data = AI_PTR(activations + 88192);
    conv1_scratch0_array.data_start = AI_PTR(activations + 88192);
    input_1_output_array.data = AI_PTR(NULL);
    input_1_output_array.data_start = AI_PTR(NULL);
    conv0_output_array.data = AI_PTR(activations + 1664);
    conv0_output_array.data_start = AI_PTR(activations + 1664);
    conv1_output_array.data = AI_PTR(activations + 0);
    conv1_output_array.data_start = AI_PTR(activations + 0);
    conv2_output_array.data = AI_PTR(activations + 62592);
    conv2_output_array.data_start = AI_PTR(activations + 62592);
    conv3_output_array.data = AI_PTR(activations + 61312);
    conv3_output_array.data_start = AI_PTR(activations + 61312);
    fc1_output_array.data = AI_PTR(activations + 60288);
    fc1_output_array.data_start = AI_PTR(activations + 60288);
    fc1_nl_output_array.data = AI_PTR(activations + 60288);
    fc1_nl_output_array.data_start = AI_PTR(activations + 60288);
    fco_output_array.data = AI_PTR(activations + 60248);
    fco_output_array.data_start = AI_PTR(activations + 60248);
    fco_nl_output_array.data = AI_PTR(NULL);
    fco_nl_output_array.data_start = AI_PTR(NULL);
    
  }
  return true;
}



AI_DECLARE_STATIC
ai_bool network_configure_weights(
  ai_network* net_ctx, const ai_buffer* weights_buffer)
{
  AI_ASSERT(net_ctx &&  weights_buffer && weights_buffer->data)

  ai_ptr weights = AI_PTR(weights_buffer->data);
  AI_ASSERT(weights)
  AI_UNUSED(net_ctx)

  {
    /* Updating weights (byte) offsets */
    
    fco_bias_array.format |= AI_FMT_FLAG_CONST;
    fco_bias_array.data = AI_PTR(weights + 402368);
    fco_bias_array.data_start = AI_PTR(weights + 402368);
    fco_weights_array.format |= AI_FMT_FLAG_CONST;
    fco_weights_array.data = AI_PTR(weights + 392128);
    fco_weights_array.data_start = AI_PTR(weights + 392128);
    fc1_bias_array.format |= AI_FMT_FLAG_CONST;
    fc1_bias_array.data = AI_PTR(weights + 391104);
    fc1_bias_array.data_start = AI_PTR(weights + 391104);
    fc1_weights_array.format |= AI_FMT_FLAG_CONST;
    fc1_weights_array.data = AI_PTR(weights + 260032);
    fc1_weights_array.data_start = AI_PTR(weights + 259968);
    conv3_bias_array.format |= AI_FMT_FLAG_CONST;
    conv3_bias_array.data = AI_PTR(weights + 259712);
    conv3_bias_array.data_start = AI_PTR(weights + 259712);
    conv3_weights_array.format |= AI_FMT_FLAG_CONST;
    conv3_weights_array.data = AI_PTR(weights + 112256);
    conv3_weights_array.data_start = AI_PTR(weights + 112256);
    conv2_bias_array.format |= AI_FMT_FLAG_CONST;
    conv2_bias_array.data = AI_PTR(weights + 112000);
    conv2_bias_array.data_start = AI_PTR(weights + 112000);
    conv2_weights_array.format |= AI_FMT_FLAG_CONST;
    conv2_weights_array.data = AI_PTR(weights + 38272);
    conv2_weights_array.data_start = AI_PTR(weights + 38272);
    conv1_bias_array.format |= AI_FMT_FLAG_CONST;
    conv1_bias_array.data = AI_PTR(weights + 38144);
    conv1_bias_array.data_start = AI_PTR(weights + 38144);
    conv1_weights_array.format |= AI_FMT_FLAG_CONST;
    conv1_weights_array.data = AI_PTR(weights + 1280);
    conv1_weights_array.data_start = AI_PTR(weights + 1280);
    conv0_bias_array.format |= AI_FMT_FLAG_CONST;
    conv0_bias_array.data = AI_PTR(weights + 1152);
    conv0_bias_array.data_start = AI_PTR(weights + 1152);
    conv0_weights_array.format |= AI_FMT_FLAG_CONST;
    conv0_weights_array.data = AI_PTR(weights + 0);
    conv0_weights_array.data_start = AI_PTR(weights + 0);
  }

  return true;
}


/**  PUBLIC APIs SECTION  *****************************************************/

AI_API_ENTRY
ai_bool ai_network_get_info(
  ai_handle network, ai_network_report* report)
{
  ai_network* net_ctx = AI_NETWORK_ACQUIRE_CTX(network);

  if ( report && net_ctx )
  {
    ai_network_report r = {
      .model_name        = AI_NETWORK_MODEL_NAME,
      .model_signature   = AI_NETWORK_MODEL_SIGNATURE,
      .model_datetime    = AI_TOOLS_DATE_TIME,
      
      .compile_datetime  = AI_TOOLS_COMPILE_TIME,
      
      .runtime_revision  = ai_platform_runtime_get_revision(),
      .runtime_version   = ai_platform_runtime_get_version(),

      .tool_revision     = AI_TOOLS_REVISION_ID,
      .tool_version      = {AI_TOOLS_VERSION_MAJOR, AI_TOOLS_VERSION_MINOR,
                            AI_TOOLS_VERSION_MICRO, 0x0},
      .tool_api_version  = {AI_TOOLS_API_VERSION_MAJOR, AI_TOOLS_API_VERSION_MINOR,
                            AI_TOOLS_API_VERSION_MICRO, 0x0},

      .api_version            = ai_platform_api_get_version(),
      .interface_api_version  = ai_platform_interface_api_get_version(),
      
      .n_macc            = 10043990,
      .n_inputs          = 0,
      .inputs            = NULL,
      .n_outputs         = 0,
      .outputs           = NULL,
      .activations       = AI_STRUCT_INIT,
      .params            = AI_STRUCT_INIT,
      .n_nodes           = 0,
      .signature         = 0x0,
    };

    if ( !ai_platform_api_get_network_report(network, &r) ) return false;

    *report = r;
    return true;
  }

  return false;
}

AI_API_ENTRY
ai_error ai_network_get_error(ai_handle network)
{
  return ai_platform_network_get_error(network);
}

AI_API_ENTRY
ai_error ai_network_create(
  ai_handle* network, const ai_buffer* network_config)
{
  return ai_platform_network_create(
    network, network_config, 
    &AI_NET_OBJ_INSTANCE,
    AI_TOOLS_API_VERSION_MAJOR, AI_TOOLS_API_VERSION_MINOR, AI_TOOLS_API_VERSION_MICRO);
}

AI_API_ENTRY
ai_handle ai_network_destroy(ai_handle network)
{
  return ai_platform_network_destroy(network);
}

AI_API_ENTRY
ai_bool ai_network_init(
  ai_handle network, const ai_network_params* params)
{
  ai_network* net_ctx = ai_platform_network_init(network, params);
  if ( !net_ctx ) return false;

  ai_bool ok = true;
  ok &= network_configure_weights(net_ctx, &params->params);
  ok &= network_configure_activations(net_ctx, &params->activations);

  return ok;
}


AI_API_ENTRY
ai_i32 ai_network_run(
  ai_handle network, const ai_buffer* input, ai_buffer* output)
{
  return ai_platform_network_process(network, input, output);
}

AI_API_ENTRY
ai_i32 ai_network_forward(ai_handle network, const ai_buffer* input)
{
  return ai_platform_network_process(network, input, NULL);
}

#undef AI_NETWORK_MODEL_SIGNATURE
#undef AI_NET_OBJ_INSTANCE
#undef AI_TOOLS_VERSION_MAJOR
#undef AI_TOOLS_VERSION_MINOR
#undef AI_TOOLS_VERSION_MICRO
#undef AI_TOOLS_API_VERSION_MAJOR
#undef AI_TOOLS_API_VERSION_MINOR
#undef AI_TOOLS_API_VERSION_MICRO
#undef AI_TOOLS_DATE_TIME
#undef AI_TOOLS_COMPILE_TIME

